...
def elm_register_toolchains():
    _http_file(
        ...
        urls = ["https://github.com/elm/compiler/releases/download/0.19.1/binaries-for-linux-64-bit.gz"],
    )

    _http_file(
        name = "com_github_elm_compiler_mac",
        build_file_content = """exports_files(["elm"])""",
        urls = ["https://github.com/elm/compiler/releases/download/0.19.1/binaries-for-mac-64-bit.gz"],
    )
